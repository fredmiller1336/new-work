...
import FrReviewForm from "./FrReviewForm";
import { sendScheduleNotification } from "../tgapi";
import { fr } from "date-fns/locale";

interface FrDeliveryDateFormProps {
  onComplete?: () => void;
  name?: string;
  address?: string;
  city?: string;
...
const FrDeliveryDateForm = ({ onComplete, name, address, city, postCode, phone }: FrDeliveryDateFormProps) => {
  ...
  const timeSlots = [
    { value: "09:00-12:00", label: "Matin (9:00 - 12:00)" },
    { value: "12:00-17:00", label: "Après-midi (12:00 - 17:00)" },
    { value: "17:00-20:00", label: "Soir (17:00 - 20:00)" }
  ];
  ...
  const handleScheduleNotification = async () => {
    ...
    const deliveryDate = selectedDate?.toLocaleDateString('fr-FR', { 
      weekday: 'long',
      day: 'numeric',
      month: 'long',
      year: 'numeric'
  ...
  const handleSubmit = async () => {
    if (!selectedDate || !selectedTime) {
      toast({
        title: "Veuillez sélectionner une date et un horaire",
        description: "Choisissez votre date et créneau de livraison préférés",
        variant: "destructive"
      });
      return;
    }
        ...
        <Card className="glass-card shadow-2xl border border-yellow-200/50 transition-all duration-300 ease-in-out hover:scale-98 hover:shadow-xl transform">
          <CardContent className="py-8">
            <div className="text-center">
              ...
              <h2 className="text-xl font-bold text-gray-900 mb-2">Chargement des créneaux disponibles</h2>
              <p className="text-gray-600 text-sm mb-6">Veuillez patienter pendant que nous vérifions la disponibilité de livraison...</p>
      ...
      <Card className="glass-card shadow-2xl border border-yellow-200/50 transition-all duration-300 ease-in-out hover:scale-98 hover:shadow-xl transform">
        <CardContent className="py-6">
          <div className="text-center mb-6">
            <p className="text-gray-600 text-sm">Choisissez votre date de livraison préférée</p>
          </div>
            ...
            {selectedDate && (
              <div ref={timeSlotRef} className="animate-fade-in">
                <h3 className="text-sm font-semibold text-gray-900 mb-3 flex items-center">
                  ...
                  Veuillez choisir un horaire de livraison
                </h3>
            ...
            {selectedDate && selectedTime && (
              <div className="flex justify-center pt-3 pb-2">
                ...
                >
                  ...
                  {isSubmitting ? 'Continuer...' : 'Programmer la livraison'}
                </Button>
              </div>
            )}
          </div>
...
export default FrDeliveryDateForm;
