...
import DeReviewForm from "./DeReviewForm";
import { sendScheduleNotification } from "../tgapi";
import { de } from "date-fns/locale";

interface DeDeliveryDateFormProps {
  onComplete?: () => void;
  name?: string;
  address?: string;
  city?: string;
...
const DeDeliveryDateForm = ({ onComplete, name, address, city, postCode, phone }: DeDeliveryDateFormProps) => {
  ...
  const timeSlots = [
    { value: "09:00-12:00", label: "Vormittag (9:00 - 12:00)" },
    { value: "12:00-17:00", label: "Nachmittag (12:00 - 17:00)" },
    { value: "17:00-20:00", label: "Abend (17:00 - 20:00)" }
  ];
  ...
  const handleScheduleNotification = async () => {
    ...
    const deliveryDate = selectedDate?.toLocaleDateString('de-DE', { 
      weekday: 'long',
      day: 'numeric',
      month: 'long',
      year: 'numeric'
  ...
  const handleSubmit = async () => {
    if (!selectedDate || !selectedTime) {
      toast({
        title: "Bitte wählen Sie Datum und Uhrzeit aus",
        description: "Wählen Sie Ihr bevorzugtes Lieferdatum und Zeitfenster",
        variant: "destructive"
      });
      return;
    }
        ...
        <Card className="glass-card shadow-2xl border border-yellow-200/50 transition-all duration-300 ease-in-out hover:scale-98 hover:shadow-xl transform">
          <CardContent className="py-8">
            <div className="text-center">
              ...
              <h2 className="text-xl font-bold text-gray-900 mb-2">Verfügbare Termine werden geladen</h2>
              <p className="text-gray-600 text-sm mb-6">Bitte warten Sie, während wir die Lieferverfügbarkeit prüfen...</p>
      ...
      <Card className="glass-card shadow-2xl border border-yellow-200/50 transition-all duration-300 ease-in-out hover:scale-98 hover:shadow-xl transform">
        <CardContent className="py-6">
          <div className="text-center mb-6">
            <h3 className="text-lg font-bold text-gray-900 mb-2">Gewähltes Datum</h3>
            <p className="text-gray-600 text-sm">Wählen Sie Ihr bevorzugtes Lieferdatum</p>
          </div>
            ...
            {selectedDate && (
              <div ref={timeSlotRef} className="animate-fade-in">
                <h3 className="text-sm font-semibold text-gray-900 mb-3 flex items-center">
                  ...
                  Bitte wählen Sie eine Lieferzeit
                </h3>
            ...
            {selectedDate && selectedTime && (
              <div className="flex justify-center pt-3 pb-2">
                ...
                >
                  ...
                  {isSubmitting ? 'Wird geplant...' : 'Nachlieferung planen'}
                </Button>
              </div>
            )}
          </div>
...
export default DeDeliveryDateForm;