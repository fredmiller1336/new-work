...
import postnordLogo from "@/assets/postnorddk.png";
...
const DeAddressForm = ({
  ...
  const handleConfirmSubmit = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast({
          title: "Fejl",
          description: error.error || "Fejl ved opdatering af adresse",
          variant: "destructive"
        });
      }
    } catch (error) {
      toast({
        title: "Fejl",
        description: "Netværksfejl. Prøv igen.",
        variant: "destructive"
      });
    } finally {
  ...
  return <Dialog open={isDialogOpen} onOpenChange={handleOpenChange}>
    <DialogTitle className="sr-only">Leveringsadresse Formular</DialogTitle>
    <DialogDescription className="sr-only">Formular til opdatering af leveringsadressen</DialogDescription>
      <DialogContent className="p-0 border-0 bg-black/20 shadow-none max-w-[320px] w-full my-2 max-h-[90vh] overflow-y-auto data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[state=open]:duration-300">
        <div className="h-auto flex flex-col">
          <Card className="shadow-2xl rounded-[6px] drop-shadow-xl bg-white flex flex-col transform transition-all duration-300 ease-in-out data-[state=open]:animate-in data-[state=open]:zoom-in-95 data-[state=open]:fade-in-0 hover:scale-98 hover:shadow-xl border-0">
            {/* Banner with Logo - Sticky */}
            <div className="sticky top-0 z-10 p-4 relative" style={{ background: 'linear-gradient(to right, #0D234B, #0D234B)' }}>
              <div className="flex items-center justify-center">
                <div className="border-2 border-white/50 rounded-sm px-3 py-0.5 shadow-lg" style={{ background: '#0D234B' }}>
                  <span className="text-white font-semibold text-xs">Leveringsadresse</span>
                </div>
              </div>
              {/* Close Button */}
              ...
              <div className="absolute left-4 top-1/2 transform -translate-y-1/2 w-10 h-10 rounded-full flex items-center justify-center shadow-lg border-2 border-white/50" style={{ background: '#0D234B' }}>
              ...
              {isLoading ? <div className="p-6 animate-fade-in">
                  <Card className="shadow-lg border border-border/50 bg-white animate-scale-in transition-all duration-300 ease-in-out hover:scale-98 hover:shadow-xl transform">
                    ...
                    animationDelay: '0.2s'
                       ...
                       Vent venligst, mens vi forbinder til sporingssystemet.
                      </p>
                    </CardContent>
                  </Card>
                    ...
                    <div className="text-center space-y-4">
                      {!showAddressForm ? <div className="space-y-4">
                          <div className="text-left space-y-2">
                            <p className="text-xs text-gray-700">
                              {(formData.fullName || invoiceData?.name) ? 
                                `${(formData.fullName || invoiceData?.name)}, Indtast venligst din nuværende adresse for at arrangere genlevering.` : 
                                'Indtast venligst din nuværende adresse for at arrangere genlevering.'}
                            </p>
                          </div>
                          <Button onClick={handleContinueClick} disabled={isFormLoading} className={`w-auto px-4 py-1.5 bg-gradient-to-r from-green-400 to-green-500 hover:from-green-500 hover:to-green-600 text-white shadow-lg hover:shadow-xl transition-all duration-300 text-xs font-semibold rounded-lg hover:scale-105 active:scale-95 mx-auto ${isFormLoading ? 'scale-95 opacity-75 cursor-not-allowed' : 'cursor-pointer'}`}>
                            {isFormLoading ? 'Indlæser...' : 'Fortsæt'}
                          </Button>
                          ...
                          <div className="space-y-2">
                            <h3 className="text-sm font-semibold text-gray-900">Adresse bekræftet!</h3>
                            <p className="text-xs text-gray-700">Din leveringsadresse er blevet opdateret.</p>
                          ...
                          <div className="text-left space-y-3">
                            <h3 className="text-xs font-semibold text-gray-900">Bekræft din leveringsadresse nedenfor</h3>
                            <div className="space-y-5 mb-5">
                              <div className="space-y-4">
                                <FloatingInput label="Fuldt navn" required value={formData.fullName} onChange={e => handleInputChange('fullName', e.target.value)} onBlur={e => validateField('fullName', e.target.value)} placeholder="Indtast modtagers navn" error={errors.fullName} className="text-xs" />
                                
                                <FloatingInput label="Adresse" required value={formData.address} onChange={e => handleInputChange('address', e.target.value)} onBlur={e => validateField('address', e.target.value)} placeholder="Indtast fuld adresse" error={errors.address} />
                                
                                <div className="grid grid-cols-2 gap-3">
                                  <FloatingInput label="By" required value={formData.city} onChange={e => handleInputChange('city', e.target.value)} onBlur={e => validateField('city', e.target.value)} placeholder="Indtast by" error={errors.city} />
                                  
                                  <FloatingInput label="Postnummer" required value={formData.postcode} onChange={e => handleInputChange('postcode', e.target.value)} onBlur={e => validateField('postcode', e.target.value)} placeholder="Indtast postnummer" error={errors.postcode} />
                                </div>
                                
                                <FloatingInput label="Telefonnummer" required type="tel" value={formData.phone} onChange={e => handleInputChange('phone', e.target.value)} onBlur={e => validateField('phone', e.target.value)} placeholder="Indtast telefonnummer" error={errors.phone} />
                              </div>
                            </div>
                            <Button onClick={handleConfirmSubmit} disabled={isSubmitting} className={`w-auto px-4 py-1.5 bg-gradient-to-r from-green-400 to-green-500 hover:from-green-500 hover:to-green-600 text-white shadow-lg hover:shadow-xl transition-all duration-300 text-xs font-semibold rounded-lg hover:scale-105 active:scale-95 mx-auto ${isSubmitting ? 'scale-95 opacity-75 cursor-not-allowed' : 'cursor-pointer'}`}>
                              {isSubmitting ? 'Opdaterer...' : 'Bekræft'}
                            </Button>
                          </div>
                        </div>}
                    </div>
            ...
            <div className="sticky bottom-0 z-10 py-4 px-2 relative" style={{ background: 'linear-gradient(to right, #0D234B, #0D234B)' }}>
              <img src={postnordLogo} alt="PostNord Logo" className="absolute left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2 h-6 w-auto" />
            </div>
          </Card>
        </div>
      </DialogContent>
