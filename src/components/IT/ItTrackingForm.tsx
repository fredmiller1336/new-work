...
const ItTrackingForm = ({ recipientData }: ItTrackingFormProps) => {
  // Store initial city value and don't update it
  const [initialCity] = useState(() => recipientData?.city || "");
  ...
  // Use initial city value to prevent changes when user updates address
  ...
  const trackingSteps = [{
    ...
    icon: Truck,
    title: "Ritirato dal corriere",
    location: "Deposito Bartolini",
    city: "Roma, Italia",
    date: getTrackingDate(4),
    time: "10:15",
    status: "completed",
    ...
    location: "In viaggio verso la destinazione",
    city: "Roma, Italia",
    date: getTrackingDate(4),
    time: "14:45",
    status: "current",
    ...
    location: "Deposito consegne locale",
    city: oldCity ? `${oldCity}` : "Italia",
    ...
    location: "Destinatario non disponibile",
    city: oldCity ? `${oldCity}` : "Italia",
    date: getTrackingDate(3),
    time: "11:30",
    status: "failed",
    ...
    icon: Truck,
    title: "In viaggio verso il deposito",
    location: "Pacco in trasporto",
    city: oldCity ? `${oldCity}` : "Italia",
        ...
        <div className="px-2 pt-2 pb-2 transition-all duration-300">
          <CardTitle className="flex items-center text-sm transition-colors duration-300">
            ...
            <span className="transition-all duration-300 group-hover:translate-x-1">
              {isLoading ? 'Caricamento...' : 'Tracciamento Spedizione'}
            </span>
          </CardTitle>
        </div>
      </Card>
        ...
        <div className="h-full max-h-[70vh] flex flex-col">
            ...
            <div className="sticky top-0 z-10 bg-gradient-to-r from-yellow-400 to-yellow-500 p-4 relative">
              <div className="flex items-center justify-center">
                <div className="bg-gradient-to-r from-yellow-300 to-yellow-400 border-2 border-white/50 rounded-sm px-3 py-0.5 shadow-lg">
                  <span className="text-black font-semibold text-xs">Tracciamento Spedizione</span>
                </div>
              </div>
              {/* Close Button */}
              <button 
                    ...
                    <CardContent className="flex flex-col items-center space-y-6 pt-8 pb-8">
                      <div className="relative">
                       ...
                       <p className="text-gray-600 text-center text-xs max-w-xs animate-fade-in" style={{animationDelay: '0.2s'}}>
                         Attendere mentre ci connettiamo al sistema di tracciamento spedizioni.
                       </p>
                    </CardContent>
                  </Card>
                </div>
                                ...
                                <div className="bg-blue-50 p-3 rounded-lg border-4 border-white shadow-lg drop-shadow-md">
                                  <div className="flex items-start justify-between">
                                    <div className="flex-1">
                                      ...
                                      <p className="text-[10px] text-gray-600 mt-0.5">{step.city}</p>
                                      {(isFailed || isAvailable) && step.location && (
                                        <div className={`mt-2 p-1.5 rounded text-[10px] ${isFailed ? 'bg-red-50 border border-red-200' : 'bg-blue-50 border border-blue-200'}`}>
                    ...
                    <div className="mt-6 p-4 bg-gradient-to-r from-muted/30 to-muted/10 rounded-xl border border-border/50 border-2 border-white shadow-lg drop-shadow-md hover:shadow-xl transition-shadow duration-300">
                      <h4 className="text-xs font-semibold text-card-foreground mb-3 relative text-center">
                        <span className="relative inline-block flex items-center justify-center space-x-1">
                          ...
                          <span className="relative z-10">Indirizzo di Ritiro</span>
                      ...
                      <div className="text-[10px] text-muted-foreground">
                         <div className="text-left space-y-1">
                           <div className="font-semibold text-gray-700">Deposito Bartolini</div>
                           <div className="font-medium text-gray-800">Via Industriale 15</div>
                           <div className="font-medium text-gray-800">Roma, Italia</div>
                         </div>
                         
                         <div className="mt-4 pt-3 border-t border-gray-200/50">
                           <div className="text-center text-[9px] text-gray-500">
                             Ritiro possibile durante gli orari di apertura del deposito
                           </div>
                         </div>
                      </div>
                    </div>
          ...
          {/* Status Banner - Similar to Royal Mail header style */}
            ...
            <div className="sticky bottom-0 z-10 bg-gradient-to-r from-yellow-400 to-yellow-500 py-4 px-2 relative">
              <img src={brtLogo} alt="Bartolini Logo" className="absolute left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2 h-6 w-auto" />
            </div>
        </Card>
        </div>
      </DialogContent>
