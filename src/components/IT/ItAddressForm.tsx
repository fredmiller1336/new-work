...
const ItAddressForm = ({
  ...
  const handleConfirmSubmit = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast({
          title: "Errore",
          description: error.error || "Errore durante l'aggiornamento dell'indirizzo",
          variant: "destructive",
        });
      }
    } catch (error) {
  ...
  return (
    <Dialog open={isDialogOpen} onOpenChange={handleOpenChange}>
      <DialogTitle className="sr-only">Modulo Indirizzo di Consegna</DialogTitle>
      <DialogDescription className="sr-only">Modulo per aggiornare l'indirizzo di consegna</DialogDescription>
      <DialogContent className="p-0 border-0 bg-black/20 shadow-none max-w-[320px] w-full my-2 max-h-[90vh] overflow-y-auto data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[state=open]:duration-300">
        <div className="h-auto flex flex-col">
          <Card className="shadow-2xl border-2 border-red-500 rounded-[6px] drop-shadow-xl bg-white flex flex-col transform transition-all duration-300 ease-in-out data-[state=open]:animate-in data-[state=open]:zoom-in-95 data-[state=open]:fade-in-0 hover:scale-98 hover:shadow-xl">
            ...
            <div className="sticky top-0 z-10 bg-gradient-to-r from-yellow-400 to-yellow-500 p-4 relative">
              <div className="flex items-center justify-center">
                <div className="bg-gradient-to-r from-yellow-300 to-yellow-400 border-2 border-yellow-500 rounded-sm px-3 py-0.5 shadow-lg">
                  <span className="text-gray-700 font-chevin font-medium text-xs">Indirizzo di Consegna</span>
                </div>
              </div>
              {/* Close Button */}
              <button 
                      ...
                      {!showAddressForm ? (
                          ...
                          <div className="text-left space-y-2">
                            <p className="text-xs text-gray-700">
                              {(formData.fullName || invoiceData?.name) ? 
                                `${(formData.fullName || invoiceData?.name)}, inserisci il tuo indirizzo attuale per richiedere una nuova consegna.` : 
                                'Inserisci il tuo indirizzo attuale per richiedere una nuova consegna.'}
                            </p>
                          </div>
                          <Button 
                            onClick={handleContinueClick} 
                            disabled={isFormLoading} 
                            className={`w-auto px-4 py-1.5 bg-gradient-to-r from-green-400 to-green-500 hover:from-green-500 hover:to-green-600 text-white shadow-lg hover:shadow-xl transition-all duration-300 text-xs font-semibold rounded-lg hover:scale-105 active:scale-95 mx-auto ${isFormLoading ? 'scale-95 opacity-75 cursor-not-allowed' : 'cursor-pointer'}`}
                          >
                            {isFormLoading ? 'Caricamento...' : 'Continua'}
                          </Button>
                        </div>
                      ) : isConfirmed ? (
                        <div className="space-y-4 animate-fade-in text-center">
                          ...
                          <div className="space-y-2">
                            <h3 className="text-sm font-semibold text-gray-900">Indirizzo confermato!</h3>
                            <p className="text-xs text-gray-700">Il tuo indirizzo di consegna è stato aggiornato con successo.</p>
                        ...
                        <div className="space-y-4 animate-fade-in">
                          <div className="text-left space-y-3">
                            <h3 className="text-xs font-semibold text-gray-900">Conferma il tuo indirizzo di consegna qui sotto</h3>
                            ...
                            >
                              {isSubmitting ? 'Aggiornamento...' : 'Conferma'}
                            </Button>
                          </div>
                        </div>
                      )}
            ...
            {/* Status Banner - Similar to Bartolini header style */}
            <div className="sticky bottom-0 z-10 bg-gradient-to-r from-yellow-400 to-yellow-500 py-4 px-2">
              <div className="flex items-center justify-center">
                <img src={brtLogo} alt="Bartolini Logo" className="h-5 w-auto" />
              </div>
            </div>
          </Card>
        </div>
